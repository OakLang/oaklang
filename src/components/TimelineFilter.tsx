...
import { defaultTimelineFilterOptions } from '~/stores/timeline-filter-options-store';
...
export default function TimelineFilter({
  ...
  onSave?: (value: TimelineFilterOptions) => void;
  value: TimelineFilterOptions;
}) {
  const [filter, setFilter] = useState<TimelineFilterOptions>(value);
  ...
  const integrationsQuery = api.integrations.allIntegrationsForUser.useQuery();
    ...
    <div>
      <div className="max-h-[512px] space-y-4 overflow-y-auto p-4">
        ...
        <div className="space-y-1">
          <Popover onOpenChange={setIntegrationPopoverOpen} open={integrationPopoverOpen}>
            <PopoverTrigger asChild>
              <Button className="w-full justify-start text-left text-foreground" id="integrations" type="button" variant="outline">
                <p className="flex-1">Sites</p>
                <p className="text-muted-foreground">{filter.integrations?.length ?? 0} selected</p>
                  ...
                  <CommandGroup heading="Sites">
                    {integrationsQuery.isLoading ? (
                      ...
                      integrationsQuery.data.map((integration) => {
                        const id = getIntegrationIdFromName(integration.name);
                        const checked = filter.integrations?.includes(id) == true;
                        return (
                          <CommandItem
                            key={id}
                            onSelect={() => {
                              if (checked) {
                                setFilter({
                                  ...filter,
                                  integrations: filter.integrations?.filter((item) => item !== id),
                                });
                              } else {
                                setFilter({
                                  ...filter,
                                  integrations: [...(filter.integrations ?? []), id],
                                });
                              }
                            }}
                            value={id}
          ...
          {filter.integrations && filter.integrations.length > 0 ? (
            <div className="flex flex-wrap gap-2 pb-4 pt-1">
              {filter.integrations.map((integration) => {
                const name = integrations.find((int) => integration === getIntegrationIdFromName(int.name))?.name;

                return (
                  <Badge className="pr-1" key={integration} variant="secondary">
                    {name}
                    <button
                      ...
                      onClick={() => {
                        setFilter({
                          ...filter,
                          integrations: filter.integrations?.filter((_integration) => _integration !== integration),
                        });
                      }}
                      type="button"
                    >
      ...
      <div className="sticky bottom-0 flex items-center justify-end gap-2 border-t px-4 py-2">
        <Button onClick={() => setFilter(defaultTimelineFilterOptions)} variant="ghost">
          Clear
        </Button>
