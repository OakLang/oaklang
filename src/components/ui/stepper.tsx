...
const StepIndicator = forwardRef<HTMLDivElement, StepIndicatorProps>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      className={cn(
        'flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border-2 border-muted bg-muted text-muted-foreground',
        {
...
const StepTrigger = forwardRef<HTMLButtonElement, StepTriggerProps>(({ className, asChild, ...props }, ref) => {
  ...
  return (
    <Comp
      className={cn(
        'flex items-center gap-4 rounded-lg p-2 pr-4 text-left ring-offset-background transition-colors hover:bg-secondary/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50',
        className,
      )}
      ref={ref}
      {...props}
...
const StepDescription = forwardRef<HTMLHeadingElement, StepDescriptionProps>(({ className, ...props }, ref) => {
  return <p className={cn('whitespace-nowrap text-sm text-muted-foreground', className)} ref={ref} {...props} />;
});
...
const StepSeperator = forwardRef<HTMLDivElement, StepSeperatorProps>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      className={cn(
        'h-2 w-0.5 rounded-full bg-border max-md:ml-[26px] md:h-0.5 md:min-w-[24px] md:flex-1',
        {
