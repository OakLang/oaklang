...
import type { GitHubRepo, GitHubRepoScrape, GitHubUser, GitLabProject, WakaTimeStats } from '~/utils/types';
import { Integration, IntegrationScrape, IntegrationScrapeRepo, ProgramLanguage } from '~/server/schema';
import { and, eq } from 'drizzle-orm';
import { getIntegrationId, getIntegrationIdFromName, wonderfulFetch } from '.';
import { formatNumberWithSuffix } from '~/utils';
import { getAccessToken, refreshTokenHandler } from './defaultHandlers';
import { siGithub, siGitlab, siStackoverflow, siWakatime } from 'simple-icons';
...
export const refreshTokenIfNeeded = async (connection: typeof Integration.$inferSelect | undefined) => {
  ...
  const id = getIntegrationId(config);
...
export const getConnectionById = async (integrationId: string) => {
  const connection = await db.query.Integration.findFirst({ where: eq(Integration.id, integrationId), with: { user: true } });
  if (connection?.user.isBannedForSpam) {
    return undefined;
  }
...
export const getTagsForConnection = async (
  ...
  switch (integration.provider) {
    ...
    case getIntegrationIdFromName(siStackoverflow.title): {
      const scrape = await db.query.IntegrationScrape.findFirst({
        where: eq(IntegrationScrape.integrationId, integration.id),
      });
      if (!scrape) {
      ...
      return (scrape.jsonValue as { answer_id?: number; api_site_parameter: string; question_id?: number; score: number; tags: string[] }[])
...
export const getProgramLanguageForGitHubRepo = async (connection: typeof Integration.$inferSelect, repo: GitHubRepo) => {
  if (repo.language) {
    return await db.query.ProgramLanguage.findFirst({ where: eq(ProgramLanguage.name, repo.language) });
  }
  if (Object.keys(repo.languages ?? {}).length > 0) {
    ...
    if (lang) {
      return await db.query.ProgramLanguage.findFirst({ where: eq(ProgramLanguage.name, lang) });
    } else {
      return;
    }
  }
