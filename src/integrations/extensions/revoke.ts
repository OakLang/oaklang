import { getIntegrationId, gitHubAppRequest, wonderfulFetch } from 'src/integrations/utils';

import type { Integration } from '~/server/schema';
import type { RevokeTokenHandler } from '~/utils/types';
...
export const GitHubRevoke: RevokeTokenHandler = async (integration: typeof Integration.$inferSelect, token: string): Promise<boolean> => {
  ...
  const integrationConfig = integrations.find((i) => {
    return getIntegrationId(i) === integration.provider;
  });
  if (!integrationConfig) {
    return false;
  }
  const secret = process.env.INTEGRATION_GITHUB_SECRET!;
  if (!secret) {
    return false;
  }
...
export const TwitchRevoke: RevokeTokenHandler = async (integration: typeof Integration.$inferSelect, token: string): Promise<boolean> => {
  ...
  const client_id = process.env.INTEGRATION_TWITCH_CLIENT_ID ?? integrationConfig.clientId;
...
export const TikTokRevoke: RevokeTokenHandler = async (integration: typeof Integration.$inferSelect, token: string): Promise<boolean> => {
  ...
  const secret = process.env.INTEGRATION_TIKTOK_SECRET!;
  if (!secret) {
    return false;
  }
  ...
  const client_id = process.env.INTEGRATION_TIKTOK_CLIENT_ID ?? integrationConfig.clientId;
