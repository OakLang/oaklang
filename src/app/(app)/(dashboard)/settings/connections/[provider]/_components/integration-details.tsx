...
export default function IntegrationDetails({ provider }: { provider: string }) {
  ...
  const integrationQuery = api.integrations.integrationForUser.useQuery({ provider });
    ...
    <>
      <div className="flex flex-col items-center justify-center border-b px-4 py-8">
        {integrationQuery.data.icon ? (
          <span
            className="mb-4 h-12 w-12 fill-foreground"
            // eslint-disable-next-line react/no-danger
            dangerouslySetInnerHTML={{ __html: integrationQuery.data.icon }}
          />
        ) : null}
        <p className="text-center text-xl font-bold">{integrationQuery.data.name}</p>
        <p className="mt-2 text-center text-muted-foreground">{integrationQuery.data.description}</p>
      ...
      {integrationQuery.data.connections.length === 0 ? (
        ...
        <Tabs value={selectedTab}>
          {tabs.length > 1 && (
            <TabsList className="sticky top-14 z-20 flex h-12 w-full border-b bg-card">
              {tabs.map((tab) => {
          ...
          {manualInfo.token && manualInfo.link ? (
            <fieldset>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Open {integrationQuery.data.name}{' '}
